...
const MODULES = [
  {
    num: '01', name: 'Dashboard',
    desc: 'Visão consolidada de indicadores, metas e performance em tempo real.',
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1"><rect x="3" y="3" width="7" height="7" /><rect x="14" y="3" width="7" height="7" /><rect x="3" y="14" width="7" height="7" /><rect x="14" y="14" width="7" height="7" /></svg>,
  },
  {
    num: '02', name: 'Captação',
    desc: 'Acompanhamento de captação líquida, metas mensais e ranking de assessores.',
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1"><polyline points="22 7 13.5 15.5 8.5 10.5 2 17" /><polyline points="16 7 22 7 22 13" /></svg>,
  },
  {
    num: '03', name: 'Performance',
    desc: 'Métricas XPerformance, histórico de resultados e análise por período.',
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1"><circle cx="12" cy="12" r="10" /><polyline points="12 6 12 12 16 14" /></svg>,
  },
  {
    num: '04', name: 'Missões',
    desc: 'Desafios ativos, pontuação da equipe e programa de reconhecimento Nobel.',
    icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" /></svg>,
  },
]

const DESTAQUES = [
  { num: 'R$ 2.4', sup: 'B', label: 'Atualizado — Mar 2026', text: 'Volume total sob gestão. Nobel Capital consolidou-se como uma das maiores escritórias XP do Brasil, com crescimento de 34% no último trimestre.' },
  { num: '48', sup: '+', label: 'Headcount — Mar 2026', text: 'Assessores ativos em expansão com 5 novas contratações previstas para Q2 2026.' },
  { num: '91', sup: '%', label: 'NPS — Q4 2025', text: 'Taxa de retenção de clientes. Índice acima da média nacional, reconhecido pelo prêmio Elite Partner 2025.' },
  { num: '12', sup: '×', label: 'Histórico — 2018–2026', text: 'Multiplicador de crescimento desde a fundação, sustentado por gestão orientada a dados.' },
]

const RECENTES = [
  { date: '17 Mar 2026', tag: 'Missões', title: 'Missão Q1 encerrada — confira os resultados e premiações' },
  { date: '14 Mar 2026', tag: 'Performance', title: 'Relatório XPerformance — Fevereiro 2026 disponível' },
  { date: '10 Mar 2026', tag: 'Captação', title: 'Novo recorde de captação líquida — R$ 180M em fevereiro' },
  { date: '05 Mar 2026', tag: 'Time', title: 'Bem-vindo ao time — 3 novos assessores iniciando em abril' },
  { date: '01 Mar 2026', tag: 'Comunicado', title: 'Calendário de reuniões gerenciais — Q2 2026 publicado' },
]
...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    /* ── nav scroll ── */
    const onScroll = () => navRef.current?.classList.toggle(styles.navScrolled!, window.scrollY > 60)
    window.addEventListener('scroll', onScroll)

    /* ── hero letter animation ── */
    const animLine = (el: HTMLSpanElement | null, text: string, delay: number, onDone?: () => void) => {
      if (!el) return
      el.innerHTML = ''
    ...
    const obs = new IntersectionObserver(
      entries => {
        entries.forEach(entry => {
          ...
          setTimeout(() => el.classList.add(styles.visible!), parseInt(el.dataset.delay ?? '0'))
          obs.unobserve(el)
        })
      },
    ...
    <div ref={rootRef} className={styles.root}>
      ...
      <nav ref={navRef} className={styles.nav}>
        ...
        <ul className={styles.navLinks}>
          ...
          <li><a href="#modulos">Time</a></li>
          <li><a href="#recentes">Missões</a></li>
          <li><a href="#recentes">Relatórios</a></li>
          <li><Link href="/login" className={styles.navCta}>Entrar</Link></li>
        </ul>
      </nav>
      ...
      <section className={styles.hero}>
        ...
        <div className={styles.heroContent}>
          <p ref={eyebrowRef} className={styles.heroEyebrow}>Sistema Interno — Acesso Restrito</p>
          ...
          <p ref={taglineRef} className={styles.heroTagline}>
            O hub central da <strong>Nobel Capital</strong> &amp; XP Investimentos.<br />
      ...
      <section id="modulos" className={styles.sectionDark}>
        <div className={styles.sectionHeader}>
          <div>
            ...
            <h2 className={styles.sectionTitle}>Módulos <em>principais</em></h2>
          </div>
          <a href="#" className={styles.sectionLink}>Ver todos <ArrowRight /></a>
        </div>
        <div className={styles.modulesGrid}>
          {MODULES.map(m => (
            <div key={m.num} className={styles.moduleCard} data-hover>
              ...
              <div className={styles.moduleArrow}>Acessar <ArrowRight /></div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="destaques" className={styles.sectionBase}>
        <div className={styles.sectionHeader}>
          <div>
            ...
            <h2 className={styles.sectionTitle}>Resultados <em>que falam</em></h2>
          </div>
        </div>
        <div className={styles.destaqueGrid}>
          {DESTAQUES.map(d => (
            <div key={d.num} className={styles.destaqueItem}>
              <div className={styles.destaqueBigNum}>{d.num}<sup>{d.sup}</sup></div>
      ...
      <section id="recentes" className={styles.sectionMid}>
        <div className={styles.sectionHeader}>
          ...
          <a href="#" className={styles.sectionLink}>Ver arquivo <ArrowRight /></a>
        </div>
        <div className={styles.recenteList}>
          {RECENTES.map(r => (
            <div key={r.title} className={styles.recenteItem} data-hover>
              ...
              <div className={styles.recenteArrow}><ArrowDiag /></div>
            </div>
          ))}
        </div>
      </section>
