...
export function CaptacaoBarChart({
  ...
  return (
    ...
    >
      {/* ── Eixo zero ── */}
      <line x1={0} y1={zeroY} x2={VW} y2={zeroY} stroke={T.zeroline} strokeWidth={1} />
      ...
      {series.map((m, i) => {
        ...
        const fill = pos ? (isLast ? T.gold : T.successDim) : isLast ? T.danger : T.dangerDim

        const labelFill = pos ? (isLast ? T.gold : T.success) : isLast ? T.danger : T.danger
        ...
        return (
          <g key={`${m.m}-${i}`}>
            {/* Barra */}
            {barH > 0.5 && <rect x={barX} y={barY} width={barW} height={barH} rx={3} fill={fill} />}
