...
const SUIT_STYLE: Record<string, React.CSSProperties> = {
  ...
  'Super Agressivo': {
    background: 'color-mix(in oklch, #8B5CF6 12%, var(--bg-elev))',
    color: '#8B5CF6',
  },
}
...
function avatarColor(id: number): string {
  return AVATAR_COLORS[id % AVATAR_COLORS.length] ?? '#2D5FA0'
}
        ...
        <div style={{ display: 'flex', gap: 3 }}>
          {STATUS_FILTERS.map(({ key, label }) => (
            <button
              ...
              style={pillBtn(statusFilter === key)}
            >
              {label}
            </button>
          ))}
                  ...
                  <td style={{ padding: '10px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        ...
                        >
                          #{c.id_cliente}
                          {c.email_cliente ? ` · ${c.email_cliente}` : ''}
                        </p>
                      </div>
                    </div>
                  </td>
      ...
      {filtered.length > 0 && (
        ...
        >
          ...
          >
            ...
            {Math.min(currentPage * PAGE_SIZE, filtered.length).toLocaleString('pt-BR')} de{' '}
            ...
            <div style={{ display: 'flex', gap: 2 }}>
              {Array.from({ length: totalPages }, (_, i) => i + 1)
                ...
                .map((p, _idx, arr) =>
                  p === '…' ? (
                    <span
                      key={`ellipsis-after-${arr[arr.indexOf(p) - 1] ?? 0}`}
                      style={{
                        fontFamily: 'var(--f-mono)',
                        fontSize: 10,
                        color: 'var(--fg-faint)',
