...
export default async function IndicadoresPage() {
  ...
  const equipesList =
    captacao && !captacao.semDados
      ? captacao.equipes.map((e) => e.equipe)
      : (receita?.equipes ?? [])

  const equipesUnificadas: EquipeUnificada[] = equipesList.map((equipe) => {
    const cap =
      captacao && !captacao.semDados ? captacao.equipes.find((e) => e.equipe === equipe) : null
  ...
  const recYtdTotal = kpis ? Object.values(kpis.recYtdByEquipe).reduce((s, v) => s + v, 0) : 0
  ...
  return (
    <div style={{ maxWidth: 1340 }}>
      ...
      {semDados ? (
        ...
        <IndicadoresTeams equipes={equipesUnificadas} total={totalUnificado} mesISO={mesISO} />
      )}
    </div>
  )
}
