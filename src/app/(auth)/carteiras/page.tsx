...
export default async function CarteirasPage() {
  ...
  const totalClientes = [...new Set(alocacao.flatMap(() => []))].length
      ...
      <div className="grid-kpi" style={{ marginBottom: 24 }}>
        ...
        <div style={{ ...card, position: 'relative' }}>
          ...
          <div style={{ padding: '16px 18px' }}>
            ...
            <span style={{ fontSize: 11, color: 'rgba(26,18,9,0.4)', padding: '2px 8px', borderRadius: 20, background: 'rgba(26,18,9,0.05)' }}>
        ...
        <div style={card}>
          <div style={{ padding: '16px 18px' }}>
            <p style={{ fontSize: 10, color: 'rgba(26,18,9,0.38)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>Maior Alocação</p>
            <p style={{ fontFamily: 'var(--font-lora, serif)', fontSize: 26, fontWeight: 600, color: '#1A1209', marginBottom: 6, lineHeight: 1 }}>
              {alocacao[0]?.produto ?? '—'}
            </p>
            <span style={{ fontSize: 11, color: 'rgba(26,18,9,0.4)', padding: '2px 8px', borderRadius: 20, background: 'rgba(26,18,9,0.05)' }}>
        ...
        <div style={card}>
          <div style={{ padding: '16px 18px' }}>
            <p style={{ fontSize: 10, color: 'rgba(26,18,9,0.38)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>Renda Fixa + Fundos</p>
            <p style={{ fontFamily: 'var(--font-lora, serif)', fontSize: 26, fontWeight: 600, color: '#1A1209', marginBottom: 6, lineHeight: 1 }}>
              {aum > 0 ? formatPct(
                (alocacao.filter(a => a.produto === 'Renda Fixa' || a.produto === 'Fundos')
                  .reduce((s, a) => s + a.total, 0) / aum) * 100
              ) : '—'}
            </p>
            <span style={{ fontSize: 11, color: 'rgba(26,18,9,0.4)', padding: '2px 8px', borderRadius: 20, background: 'rgba(26,18,9,0.05)' }}>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  gap: 16,
                  padding: '10px 20px',
                  borderBottom: i < alocacao.length - 1 ? '1px solid rgba(184,150,62,0.06)' : 'none',
                }}
              >
                {/* Produto */}
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <div style={{ width: 10, height: 10, borderRadius: 3, background: color, flexShrink: 0 }} />
                  <span style={{ fontSize: 13, fontWeight: 500, color: '#1A1209', whiteSpace: 'nowrap' }}>
                    {item.produto}
                  </span>
                </div>
