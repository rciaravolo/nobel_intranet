...
): Promise<{ events: GridEvent[]; source: string }> {
...
export default async function AgendaPage() {
  ...
  const todayEvents = events.filter(
    (e) => e.day === 0 || weekDays[e.day]?.toDateString() === today.toDateString(),
  )
  ...
  return (
    <div style={{ maxWidth: 1200 }}>
      ...
      {source === 'unconfigured' && (
        ...
        >
          <strong>Para sincronizar com o Outlook:</strong> adicione{' '}
          <code
            style={{
          ...
          no <strong>.env.local</strong> e faça redeploy.
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 280px', gap: 20 }}>
                            ...
                            return (
                              ...
                              >
                                ...
                                {ev.duration >= 0.75 && ev.where && (
                                  <div style={{ fontSize: 10, color: ev.textColor, opacity: 0.7 }}>
                                    {ev.where}
                                  </div>
                                )}
                              </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          >
            ...
            {(() => {
              const evs = todayIdx !== -1 ? events.filter((e) => e.day === todayIdx) : []
              if (evs.length === 0) {
                return (
