...
export function BlocoMetaReceita({ filterType, filterValue }: Props) {
    ...
    return (
      <div style={cardStyle}>
        <div style={headerStyle}>
          <span style={titleStyle}>Meta de Receita — {mesLabelAtual()}</span>
        </div>
      ...
      <div style={cardStyle}>
        ...
        >
          <span style={{ fontSize: 12, color: 'var(--color-negative)' }}>
            Erro ao carregar meta de receita.
          </span>
          <button
            type="button"
            onClick={() => refetch()}
  ...
  const { realizado, meta } = data.receita
          ...
          <div>
            ...
            {realizado !== 0 && (
              <p style={{ fontSize: 11, color: 'var(--fg-faint)' }}>
                Receita realizada no mês: {fBRL(realizado)}
              </p>
            )}
          </div>
        </div>
