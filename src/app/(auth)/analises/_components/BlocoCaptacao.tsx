          ...
          {dados.map((r) => (
            <tr key={r.id_cliente} style={{ borderTop: '1px solid var(--line)' }}>
              <td style={{ padding: '7px 0', fontSize: 12 }}>
                ...
                <span
                  style={{ fontSize: 10, color: 'var(--fg-faint)', fontFamily: 'var(--f-mono)' }}
                >
                  #{r.id_cliente}
                  {r.nome_assessor && (
                    <>
...
export function BlocoCaptacao({ captacao, mesLabel, filterType, filterValue }: Props) {
  ...
  const items = [
    ...
    { label: 'Resgates', value: captacao.resgates, up: false, clickable: false },
    {
      ...
      value: captacao.liquida,
      up: captacao.liquida >= 0,
      clickable: true,
    },
  ]
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', padding: '20px' }}>
        {items.map((item, i) => (
          <div
            ...
            onMouseEnter={(e) => {
              if (item.clickable) e.currentTarget.style.background = 'var(--n-50)'
            }}
            onMouseLeave={(e) => {
              if (item.clickable) e.currentTarget.style.background = 'transparent'
            }}
          >
            <p
              style={{
            ...
            >
              ...
              {item.clickable && (
                <span
                  style={{ marginLeft: 6, fontSize: 12, fontWeight: 600, color: 'var(--c-gold)' }}
                >
                  {loading ? '...' : open ? '▲' : '▼'}
                </span>
              )}
            ...
            <span style={pill(item.up)}>
              {item.up ? '↑' : '↓'}{' '}
              {item.label === 'Captação Líquida' ? 'resultado do mês' : item.label.toLowerCase()}
            </span>
          </div>
        ))}
      </div>
      ...
      {erro && (
        <div
          style={{
            ...
            fontSize: 12,
            borderTop: '1px solid var(--color-negative)',
          }}
        >
